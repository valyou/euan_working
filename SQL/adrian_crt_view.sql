drop view adrian_category_totals;
create view adrian_category_totals as
(
with
aboriginal as
(
select cat.gid, cc.parent_category_code category_code, string_agg(cat.description, '. ') as description, min(cat.raw_value) raw_value, round(avg(cat.value)) as value
from category cc
inner join category_values cat on (cc.category_code = cat.category_code)
where parent_category_code = 'ABORIGINAL'
group by cat.gid, cc.parent_category_code
),
conservation as
(
select cat.gid, cc.parent_category_code category_code, string_agg(cat.description, '. ') as description, min(cat.raw_value) raw_value, round(avg(cat.value)) as value
